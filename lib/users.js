...
export async function upsertUser({ email, name, avatar, googleId, country, refreshToken }) {
  const updatePayload = {
    email,
    name,
    avatar,
  ...
  // Only overwrite the stored refresh token if Google actually issued a new one —
  // it's only sent on first consent, not every login, so don't null it out otherwise.
  if (refreshToken) updatePayload.google_refresh_token = refreshToken

  const { data, error } = await supabase
    .from('users')
    .upsert(updatePayload, { onConflict: 'google_id' })
    .select()
    .single()
...
export async function getSenderProfile(googleId) {
  const { data, error } = await supabase
    .from('users')
    .select('company_name, sender_name, value_prop, proof_point, email_signature, target_roles, onboarded, name')
    .eq('google_id', googleId)
    .single()
...
export async function saveSenderProfile(googleId, { companyName, senderName, valueProp, proofPoint, emailSignature, targetRoles }) {
  const { error } = await supabase
    .from('users')
    .update({
